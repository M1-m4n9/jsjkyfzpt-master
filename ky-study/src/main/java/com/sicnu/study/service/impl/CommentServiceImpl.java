...
import com.sicnu.study.feign.PersonFeignService;
import com.sicnu.study.feign.vo.UserCenterVo;
...
import com.sicnu.study.dao.CommentDao;
...
import static com.sicnu.common.utils.Constant.LIKE_RECORD;
...
//		return new PageUtils(page);
		...
		for (CommentEntity record : records)
		{
			...
			String likeCount = rediskeycon(record.getId(), STUDY_COMMENT);
			...
			/********************************************************/
			if (uidl!=null) {
				CollectionUserEntity cuEntity = new CollectionUserEntity();
				cuEntity.setUid(Integer.parseInt(String.valueOf(uidl)));
				cuEntity.setCollectionId(record.getId());
				cuEntity.setCollectionType(2);
				Integer cliked = collectionUserService.selectByUTT(cuEntity).getLiked();
			...
			R r = personFeignService.simpleInfo(uid);
			if (r.getCode() == 0)
			{
				...
				if (true)//entityType == 1 && Integer.parseInt(targetId) != 0 && entityId.intValue() != Integer.parseInt(targetId)
				{
					...
					R r1 = personFeignService.simpleInfo(byIdUid);
					if (r1.getCode() == 0)
					{
...
//                List<Integer> sonIds = this.list(lqw).stream().map(CommentEntity::getEntityId).collect(Collectors.toList());
			...
			CommentEntity commentEntity1 = baseMapper.selectById(commentId);
			commentEntity1.setIsDeleted(1);
			baseMapper.updateById(commentEntity1);
		});
	}
	...
	{
		Integer collectionId=Integer.parseInt(String.valueOf(params.get(COLLECTION_ID)));
		Object uidl = params.get(UID);
		LambdaQueryWrapper<CommentEntity> lqw = new LambdaQueryWrapper<>();
...
//        List<CommentVo> commentVos = new ArrayList<>();
		...
		commentEntities.stream().forEach(commentEntity -> {
			...
			String likeCount = rediskeycon(commentEntity.getId(), STUDY_COMMENT);
			if (Boolean.TRUE.equals(redisTemplate.hasKey(likeCount))) {
				Object o = redisTemplate.opsForHash().get(likeCount, LIKE_RECORD);
				if(o!=null){
					int likec = Integer.parseInt(String.valueOf(o));
			...
			if (uidl!=null) {
				CollectionUserEntity cuEntity = new CollectionUserEntity();
				cuEntity.setUid(Integer.parseInt(String.valueOf(uidl)));
				cuEntity.setCollectionId(commentEntity.getId());
				cuEntity.setCollectionType(2);
				Integer cliked = collectionUserService.selectByUTT(cuEntity).getLiked();
...
//        for (CommentVo commentVo : notTopCommentVo) {list.add(commentVo);}
	...
	public boolean saveComment(CommentEntity commentEntity) {
		try {
			...
			String redisc = rediskeycon(id, STUDY_COMMENT);
